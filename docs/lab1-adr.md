...
NorthStar Retail is building one AI platform to host three systems: a weekly batch churn model, a real-time RAG offer generator, and an agentic customer-service system. The business case is a $128.5M annual churn problem (2.1M active customers x 18% churn x $340 LTV), and the first deliverable is a churn model that must produce scores by Monday 6 AM ET every week. Before anything can be trained, the platform needs somewhere to run notebooks, somewhere to keep data, and rules about who may touch which data.

Those rules cannot be bolted on later. The platform will ingest customer PII subject to GDPR and CCPA, and the offer system's credit component falls under FCRA/ECOA. If raw customer data and trained models share one undifferentiated bucket and one all-powerful role, there is no way to later prove that the training identity never read raw PII. Lab 1 therefore fixes the storage tiers and identity boundaries now, while there is one role and zero data, so every later lab inherits them instead of retrofitting them. The foundation must also cost nearly nothing idle: the account has $200 of credit for six months.

### Decision

**Network.** One VPC, `northstar-dev-vpc` (10.0.0.0/16), with a single public subnet `northstar-dev-public-1` (10.0.100.0/24) in us-east-1a, an attached Internet Gateway, and a route table sending 0.0.0.0/0 to it. Studio runs in that subnet behind `northstar-dev-sagemaker-sg` (inbound from 10.0.0.0/16 only, all outbound) with `PublicInternetOnly` access, reaching S3 and ECR through the IGW. Starting the public subnet at .100 leaves the low ranges free for Lab 2's private subnets.

**Storage.** One bucket, `northstar-dev-data-829485866627`, with four prefixes: `raw/`, `processed/`, `features/`, `artifacts/`. Versioning on, SSE-S3 on, all public access blocked. The prefixes are the data lifecycle: POS and Shopify exports land in `raw/`, cleaned tables in `processed/`, the weekly churn feature set in `features/`, models and evaluation output in `artifacts/`. Versioning exists because the churn model retrains weekly; a retrain that silently overwrote last week's features would make a bad Monday score impossible to diagnose or roll back.
...
- **Auditing PII exposure.** "Which identities can read raw customer data?" is answered by grepping policies for `raw/*`. Today: none.
...
- **Rebuilding from scratch.** Every name derives from `project` and `environment`; the same modules produce `northstar-local-*` on LocalStack unchanged.
...
- **Prefix-level IAM is coarse.** It cannot express "features for Canadian customers only." GDPR data-subject requests will need a catalog layer, not S3 prefixes.
- **The policy contains `sagemaker:RegisterModel`**, which is not a real IAM action (the console rejects it; course staff asked that it stay as specified). Model Registry writes will need `CreateModelPackage` before Lab 3 registers a model version.
...
**Four buckets instead of four prefixes** (`northstar-dev-raw`, `-processed`, `-features`, `-artifacts`) with bucket-level IAM. This is workable and easier to audit: bucket policies are simpler than prefix ARNs, and a trailing-wildcard mistake cannot expose a sibling stage. It was rejected because the churn pipeline moves data between stages weekly: cross-bucket copies cost more, the 24-month retention rule would have to be maintained in four places, and four times as many globally unique names must be reserved across dev/staging/prod. One bucket with strict prefix policies gives the same isolation with one retention policy and one access log.
...
- **Storage design:** Amazon S3 with stage prefixes, versioning and SSE-S3, because the churn model's inputs and outputs are batch files and S3 is what every SageMaker service reads natively.
...
- **ML development environment:** SageMaker Studio, because it runs as the MLEngineer role inside the VPC, feeds directly into the training jobs and registry later labs use, and bills only while a space is running.
